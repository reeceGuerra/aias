...
- **Framework canonical**: `aias/.canonical/subagents/<name>.md` — read-only, shipped with the framework
- **Project-owned copy**: `aias-config/subagents/<name>.md` — generated/customizable by the adopter; regenerated by `aias generate --shortcuts`
- **Tool-specific shortcuts**: `.cursor/agents/<name>.md` — symlinks to `aias-config/subagents/` created by `aias init`/`aias generate --shortcuts` (Cursor tool only)
