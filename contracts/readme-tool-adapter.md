# Tool Adapter Contract — Rho AIAS Configuration System (v1.1)
...
| Commands (command-shaped skills) | `aias/.skills/*/SKILL.md` | `aias-config/skills/*/SKILL.md` |
| Skills | `aias/.skills/*/` | `aias-config/skills/*/` |
| Providers | — | `aias-config/providers/` |
| Context | — | `RHOAIAS.md` (project root) |

Rules and modes have no framework location — they are always generated per-project from canonical templates. Commands are implemented as command-shaped advisory/operative skills from `aias/.skills/` and `aias-config/skills/`; the generator discovers both sources and creates shortcuts for all.

---

## Architectural Invariants

Tool adapters MUST preserve these invariants:

| Invariant | Contract rule | Rationale |
|---|---|---|
| Workspace-only paths | Shortcuts MUST resolve within repository/workspace scope (`<repo>/...`). User-home global targets (`~/...`) are forbidden. | Ensures reproducible, repo-local behavior for all contributors. |
| Canonical indirection only | Shortcuts MUST point to canonical AIAS sources (`aias/`, `aias-config/`, `RHOAIAS.md`) and MUST NOT embed duplicated canonical content. | Prevents drift and duplicate maintenance surfaces. |
| Deterministic generation | Generator output MUST be idempotent and derivable only from canonical sources + selected tools in stack profile. | Keeps shortcut trees auditable and scriptable. |

### MCP and User-Scoped Configuration — Out of Scope

- Tool-adapter contracts govern shortcut generation only.
- MCP transport configuration, authentication secrets, and per-user MCP runtime state are **out of scope** for tool adapters.
- User-scoped editor/global directories are **out of scope** (`~/.cursor/...`, `~/.config/...`) unless explicitly declared by a separate contract.
...
| Commands | `.cursor/skills/<name>/SKILL.md` | Symlink | Symlink → `aias/.skills/<name>/SKILL.md` or `aias-config/skills/<name>/SKILL.md` |
...
| Commands | `.github/agents/<name>.md` | Symlink | Symlink → `aias/.skills/<name>/SKILL.md` or `aias-config/skills/<name>/SKILL.md` |
| Skills | Not supported | — | — |
...
| Modes | Not supported | — | — |
| Commands | `.codex/commands/<name>.md` | Symlink | Symlink → `aias/.skills/<name>/SKILL.md` or `aias-config/skills/<name>/SKILL.md` |
...
```
.cursor/skills/implement/SKILL.md → aias/.skills/implement/SKILL.md   (symlink)
```

**Commands (GitHub Copilot — Agents):**

```
.github/agents/implement.md → aias/.skills/implement/SKILL.md   (symlink)
```

**Commands and Skills (Codex):**

```
.codex/commands/implement.md → aias/.skills/implement/SKILL.md      (symlink)
...
4. Discovers command-shaped skills from two sources: framework (`aias/.skills/`) and project (`aias-config/skills/`), generating shortcuts for both
...
| Tool | SupportTier | Rules | Modes | Commands | Skills | Context | MCP (Out-of-Scope) | SubAgents | Notes |
|---|---|---|---|---|---|---|---|---|---|
| Cursor | Primary | Y | Y | Y | Y | Y | N | Y | Native `.mdc` + symlink fidelity; globs preserved |
| Claude Code | High | Y | Y | N | Y | Y | N | Y | Enriched text rules/modes; symlink skills/context |
| Windsurf | Limited | Y | N | N | N | Y | N | N | Rules only (always-apply); 12k limit |
| GitHub Copilot | High | Y | Y | Y | N | Y | N | Y | Rule aggregation + applyTo mode files + command symlinks |
| Codex | Moderate | N | N | Y | Y | Y | N | Y | Commands/skills/context only; rules/modes unsupported |
| Gemini | Context-only | N | N | N | N | Y | N | N | Context symlink only (`GEMINI.md`) |
