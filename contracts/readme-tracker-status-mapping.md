...
> **Note:** `pending_dor → ready` is a **manual transition** (team responsibility during refinement). The framework does not automate this transition. `/enrich` publishes artifacts and posts a brief comment for team context, but does not change the tracker status.
