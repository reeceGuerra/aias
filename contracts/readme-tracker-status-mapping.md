...
- `command_triggers` (map of canonical trigger key -> canonical transition)
...
- `slash + kebab-case` with optional variant suffix `@variant-kebab-case` (regex: `^/[a-z][a-z0-9-]*(?:@[a-z][a-z0-9-]*)?$`)
...
See also: `aias/.skills/rho-aias/reference.md` § Tracker Mapping Resolution for operational enforcement.
...
| Trigger keys use canonical trigger-key format (`/command` or `/command@variant`) | ☐ |
