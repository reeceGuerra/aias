...
4. Follow these rules:
   ...
   - **Plan Artifacts — TODO `kind` enum (v9.5+) + Refinement Artifact Mutation Invariant (v9.6+)** — When a new command interacts with `technical.plan.md` TODOs, it MUST honor the closed `kind` enum: `validation | amendment_dor | amendment_dod` (absent `kind` is treated as `validation` for backward compatibility). The enum is documented in `aias/.skills/rho-aias/reference.md § Todo kind enum`. The flow is split between two commands and MUST NOT be re-implemented elsewhere: `/validate-plan` v2.1.0+ **registers** TODOs (no decisions, no apply; parses multi-line bullets and preserves the `**Inline confirmation**:` sub-field marker); `/consolidate-plan` v2.1.0+ **resolves** TODOs by `kind` and routes the change to the correct target artifact (`validation` → named artifact; `amendment_dor` → `dor.plan.md`; `amendment_dod` → `dod.plan.md`). The legacy combined `## Proposed DoR/DoD Amendments` section is FORBIDDEN — `/blueprint` emits two separate sections (`## Proposed DoR Amendments`, `## Proposed DoD Amendments`) and `/validate-plan` hard-fails when the combined block is detected. Do NOT invent new `kind` values without updating `readme-artifact.md` and `reference.md` first. The TODO lifecycle is `pending → completed | deleted-from-frontmatter` — there is no `cancelled` terminal state. **Refinement Artifact Mutation Invariant (v9.6+)** (`aias/contracts/readme-artifact.md` v2.3 § Refinement Artifact Mutation Invariant): only `/enrich` and `/blueprint` (bug exception only: `profile: bugfix` + `feasibility.assessment.md` + DoR/DoD missing) MAY CREATE `dor.plan.md` / `dod.plan.md`; only `/enrich --refresh` and `/consolidate-plan` MAY MODIFY them. New commands MUST NOT mutate DoR/DoD even with explicit user confirmation in chat — capture user answers as a `**Inline confirmation**: <value> (YYYY-MM-DD)` sub-field marker inside a Proposed Amendment bullet instead. The `## Resolution Log` heading is deprecated (legacy v9.5 artifacts MAY keep existing logs; new emissions are FORBIDDEN). When designing tracker re-read flows, use the `--refresh` flag pattern on `/enrich` rather than inventing a sibling command; `expand` parameter for Atlassian `getJiraIssue` is a comma-separated string, not an array.
...
3. Follow these rules:
   ...
   - **Canonical Section Titles invariant (v9.4+)** — Producer skills (skills that write task artifacts) MUST declare the canonical heading shape for every artifact they emit, and MUST NOT bleed internal data-collection labels (e.g., `## Category N: <title>`, `## Phase N: <title>`, `## Step N: <title>`) into the produced artifact. The contract is in `aias/contracts/readme-artifact.md § Canonical Section Titles` (v2.1+). When adding a new producer skill, populate the `Canonical headings per artifact type` table and add a `### Canonical Section Titles` sub-section under § Content Rules referencing the verbatim headings the skill emits.
...
**Canonical sub-agents** (maintained in `aias/.canonical/subagents/`) are framework-level defaults. They are copied to `aias-config/subagents/` (project-owned generated surface) by `aias init`/`aias generate --shortcuts`. To customize, edit the files in `aias-config/subagents/` — changes survive framework updates until the next regeneration (review `git diff` to decide what to keep).
...
1. Create `aias/.canonical/subagents/<name>.md` (framework canonical) or edit `aias-config/subagents/<name>.md` (project-owned copy). For project-level agents not generated by the framework, add directly to `.cursor/agents/<name>.md`.
