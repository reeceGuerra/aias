...
**Sub-agents** are a related but distinct artefact. They follow a three-tier projection model:
- **Framework canonical** (`aias/.canonical/subagents/`) — read-only defaults shipped with the framework.
- **Project-owned copies** (`aias-config/subagents/`) — generated/customizable by the adopter team; regenerated by `aias generate --shortcuts` or `aias init`. This is a _project-owned generated surface_: editable between framework updates, with Git as the mechanism for reviewing overwrites.
- **Tool shortcuts** (`.cursor/agents/`) — symlinks pointing to `aias-config/subagents/`, created by `aias init`/`aias generate --shortcuts`.
