...
#   aias/.canonical/subagents/ (framework read-only)
#   → aias-config/subagents/  (project-owned copies, customizable; regenerated by aias generate --shortcuts)
#   → .cursor/agents/         (tool-specific symlinks pointing to aias-config/subagents/)
FW_AGENTS_DIR = ROOT / "aias" / ".canonical" / "subagents"
SUBAGENTS_OUTPUT_DIR = AIAS_CONFIG_DIR / "subagents"
REVIEW_SUBAGENTS = (
...
# Legacy source paths flagged by health
LEGACY_SUBAGENT_SOURCES = (
    "aias/.cursor/agents/",
    "aias/.canonical/subagents/",
)
...
def _ensure_review_subagent_symlinks() -> None:
    """Copy sub-agents to aias-config/subagents/ and create .cursor/agents/ symlinks.

    Three-tier projection:
      aias/.canonical/subagents/ → aias-config/subagents/ → .cursor/agents/
    aias-config/subagents/ is project-owned and regenerable; the adopter should
    review git diff after generation.
    """
    ...
    for agent_name in REVIEW_SUBAGENTS:
        fw_source = FW_AGENTS_DIR / agent_name
        if not fw_source.is_file():
            print(f"  [WARN] Framework sub-agent not found: {fw_source.relative_to(ROOT)}")
            skipped += 1
            continue
        # Copy from framework canonical to project-owned directory
        config_dest = SUBAGENTS_OUTPUT_DIR / agent_name
        config_dest.write_text(fw_source.read_text(encoding="utf-8"), encoding="utf-8")
        copied += 1
        # Create symlink from .cursor/agents/ → aias-config/subagents/
        _create_symlink_for_path(agents_dir / agent_name, config_dest)
        linked += 1
    print(f"\n  Review sub-agents: {copied} file(s) written to aias-config/subagents/, "
          f"{linked} symlink(s) refreshed in .cursor/agents/"
...
def _check_review_subagent_integrity(results: List[HealthStatus]) -> None:
    """Validate the three-tier sub-agent chain for all 6 BL-S53 review sub-agents.

    Checks:
    - aias-config/subagents/*.md exist (project-owned copies)
    - .cursor/agents/*.md are symlinks pointing to aias-config/subagents/
    - Legacy symlinks pointing to old locations are flagged
    - Frontmatter invariants on the resolved files
    """
    agents_dir = ROOT / ".cursor" / "agents"
    ...
    for agent_name in REVIEW_SUBAGENTS:
        # Tier 2: aias-config/subagents/
        config_file = SUBAGENTS_OUTPUT_DIR / agent_name
        if not config_file.is_file():
            missing_config.append(agent_name)

        # Tier 3: .cursor/agents/ symlinks
        link = agents_dir / agent_name
        if not link.exists():
            missing_link.append(agent_name)
        elif link.is_symlink():
            target = os.readlink(link)
            # Flag symlinks that still point to old framework locations
            if "aias-config/subagents" not in target and "aias-config\\subagents" not in target:
                legacy_links.append(agent_name)

        # Frontmatter validation (resolve tier-3 symlink to tier-2 file)
        if link.exists():
    ...
    if not missing_config:
        results.append(("Review sub-agents (config)", "OK",
                        f"All {len(REVIEW_SUBAGENTS)} sub-agents present in aias-config/subagents/"))
    else:
        preview = "; ".join(missing_config[:3])
        suffix = f" (+{len(missing_config) - 3} more)" if len(missing_config) > 3 else ""
        results.append(("Review sub-agents (config)", "WARN",
                        f"Missing in aias-config/subagents/: {preview}{suffix}. Run: aias init"))

    if not missing_link:
        results.append(("Review sub-agents (shortcuts)", "OK",
                        f"All {len(REVIEW_SUBAGENTS)} symlinks present in .cursor/agents/"))
    else:
        preview = "; ".join(missing_link[:3])
        suffix = f" (+{len(missing_link) - 3} more)" if len(missing_link) > 3 else ""
        results.append(("Review sub-agents (shortcuts)", "WARN",
                        f"Missing symlink(s) in .cursor/agents/: {preview}{suffix}. Run: aias generate --shortcuts"))

    if legacy_links:
        preview = "; ".join(legacy_links[:3])
        results.append(("Review sub-agents (legacy)", "WARN",
                        f"Legacy symlink target(s): {preview}. Run: aias generate --shortcuts to re-target to aias-config/subagents/"))
...
def cmd_health() -> None:
    ...
    config_dirs = [RULES_DIR, MODES_DIR, PROVIDERS_DIR, SUBAGENTS_OUTPUT_DIR]
            ...
            for link in cursor_rules.iterdir():
                if link.is_symlink():
                    ...
                    if "aias/.rules/" in target or "aias/.modes/" in target:
                        ...
                        break
        # Detect legacy sub-agent symlinks pointing to old framework location
        cursor_agents = ROOT / ".cursor" / "agents"
        if cursor_agents.is_dir():
            for link in cursor_agents.iterdir():
                if link.is_symlink():
                    target = str(os.readlink(link))
                    if "aias/.cursor/agents/" in target or (
                        "aias/.canonical/subagents/" in target and "aias-config" not in target
                    ):
                        results.append(("Legacy sub-agent shortcuts", "WARN",
                            "Sub-agent symlinks point to legacy location. Run aias generate --shortcuts to re-target to aias-config/subagents/"))
                        break
