...
COMMANDS AND SKILLS
- When the user invokes a slash command, follow the command definition strictly. Commands (as advisory/operative skills) are loaded from `aias/.skills/` or `aias-config/skills/` — they are NOT rules. Do not search rule directories for commands.
